...
function tryTakeChips(source,amount)
	...
	local item = xPlayer.getInventoryItem('jeton').count
		if item >= amount then
			...
			TriggerClientEvent('esx:showNotification', source, 'Yeterli jetonunuz yok')
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent("blackjack:notify",source,"Hata, Oturamazsın.")
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 15000 then
                    TriggerClientEvent("blackjack:notify",source,"15k üzeri bahisler kabul edilmez.")
                else 
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent("blackjack:notify",source,  tostring(betAmount)  " jeton bahis yapıldı "  )
                    end
                end
            end
        end
    else
        TriggerClientEvent("blackjack:notify",source,"Bahis hatası")
    end
end)
