...
RMenu:Get('casino_enter', 'casino'):SetSubtitle("~b~GİRİŞ")
RMenu.Add('casino_exit', 'casino', RageUI.CreateMenu("", "",0,0,"shopui_title_casino", "shopui_title_casino"))
RMenu:Get('casino_exit', 'casino'):SetSubtitle("~b~ÇIKIŞ")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('casino_exit', 'casino')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.Button("Kumarhaneden Çık", nil,{ RightLabel = "→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.Button("Kumarhaneye Gir", nil,{ RightLabel = "→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
